...
| Revision 15 | Reservation Calendar Nullable FK Hardening, Timezone Sync & Collision Overhaul | 1. Resolved backend crash (HTTP 500 ValidationError) when serializing appointments with NULL `patient_id` or `dentist_id` (from `ON DELETE SET NULL` foreign key cascades) by making `patient_id`, `dentist_id`, `created_at`, and `updated_at` nullable (`Optional[str] = None`) and adding datetime-to-ISO pre-validators in `AppointmentResponse`. 2. Eliminated UTC date-shift drift on `/reservations` by introducing timezone-safe `getLocalDateString()` for initial state and day navigation. 3. Upgraded `calendar-collision.ts` `parseTimeToHour` to support composite range strings (e.g., `'09:00 AM › 10:00 AM'`) and midnight boundaries without collapsing card heights. 4. Hardened doctor matching via `extractDoctorNumber` mapping `DEN-000001`, `DOC-000001`, and `d1` to appropriate practitioner columns (Dr. Sarah Wilson, Dr. Michael Chen, ROHAN). 5. Wired live event dispatchers and listeners (`'appointment-created'`, `'appointment-updated'`) to trigger immediate calendar refreshes without manual reload. 6. Hardened `.gitignore` across all projects with strict `.env` exclusions. |
